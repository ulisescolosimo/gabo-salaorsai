-- Agregar campo email_template a la tabla shows
-- Este campo almacenará el template HTML personalizado para cada evento

ALTER TABLE public.shows
ADD COLUMN email_template TEXT;

-- Comentario descriptivo para el campo
COMMENT ON COLUMN public.shows.email_template IS 'Template HTML personalizado para el email de confirmación. Si es NULL, se usa el template por defecto.';

-- Template HTML por defecto que se puede usar como base
-- Este es el template actual de n8n, pero ahora se puede personalizar por show
UPDATE public.shows
...
SELECT id, titulo, 
       CASE 
         WHEN email_template IS NULL THEN 'Sin template'
         WHEN LENGTH(email_template) > 0 THEN 'Con template'
         ELSE 'Vacío'
       END as estado_template
FROM public.shows;
